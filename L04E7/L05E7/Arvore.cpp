...
Arvore::~Arvore()
{
	...
	Arvore* aux = new Arvore();  	if (!eVazia())
	{
		if (raiz->getEsquerdo() != NULL) {//se há uma subárvore à esquerda...  	 	 
			aux->raiz = raiz->getEsquerdo();  	 	 	
			aux->~Arvore();//...remover essa árvore 
		}
		if (raiz->getDireito() != NULL) {//se há uma subárvore à direita...  	 
...
void Arvore::remove(int elem)
{
	...
	else  	 	 	 	raiz = NULL; //se o não tem pai, o nó é a raiz.  
...
void Arvore::geraArvore(int nElementos, int limite)
{
	...
	No* Arvore::procura(int elem)
	{
		...
		if (raiz->getInfo() == elem)   return raiz;//o elemento está na raiz. 
		else
		{
	...
	No* Arvore::procuraPai(No * umNo)
	{
		...
		Arvore* pAux = new Arvore;  	pAux->raiz = raiz;

		if (pAux->raiz->getEsquerdo() == umNo || pAux->raiz->getDireito() == umNo)  	 	return pAux->raiz;// o pai é a raiz. 

		pAux->raiz = raiz->getEsquerdo();
		No* pN;
		if ((pN = pAux->procuraPai(umNo)) != NULL)  	 	return pN; //procura o pai na subárvore esquerda  	else 
	...
	void ArvoreDeBusca::insere(int elem)
		{
			if (existe(elem))
				return; //insere não admite a inserção de elementos repetidos
			if (eVazia())
			...
			do {//procura repetidamente um nó vazio, com a preferência dada 
				
				if (pAux->getInfo() > elem)//compara 
				{
						if (pAux->getEsquerdo() != NULL)//o nó esquerdo não está vazio 
						{
							pAux = pAux->getEsquerdo();//procura agora a partir dele 
				...
				{
					if (pAux->getDireito() != NULL)//nó direito nao esta vazio
					{
						pAux = pAux->getDireito();
						continue;
					}
					pAux->setDireito(new No(elem));// insere elemento nó direito
					break; //encerra a procura por um noh vazio 
